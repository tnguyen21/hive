...
def test_note_deliveries_table_created(temp_db):
    """Verify note_deliveries table exists after migration."""
    cursor = temp_db.conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='note_deliveries'")
    assert cursor.fetchone() is not None


def test_create_note_deliveries_agent_global(temp_db):
    """create_note_deliveries creates agent-global rows (recipient_issue_id=NULL)."""
    note_id = temp_db.add_note(content="broadcast note")
    count = temp_db.create_note_deliveries(note_id, to_agents=["agent-1", "agent-2"])
    assert count == 2

    rows = temp_db.conn.execute(
        "SELECT recipient_agent_id, recipient_issue_id FROM note_deliveries WHERE note_id = ? ORDER BY recipient_agent_id",
        (note_id,),
    ).fetchall()
    assert len(rows) == 2
    assert rows[0]["recipient_agent_id"] == "agent-1"
    assert rows[0]["recipient_issue_id"] is None
    assert rows[1]["recipient_agent_id"] == "agent-2"
    assert rows[1]["recipient_issue_id"] is None


def test_create_note_deliveries_issue_target(temp_db):
    """create_note_deliveries creates issue-following target rows (recipient_agent_id=NULL)."""
    note_id = temp_db.add_note(content="issue note")
    issue_id = temp_db.create_issue("Test issue")
    count = temp_db.create_note_deliveries(note_id, to_issues=[issue_id])
    assert count == 1

    rows = temp_db.conn.execute(
        "SELECT recipient_agent_id, recipient_issue_id FROM note_deliveries WHERE note_id = ?",
        (note_id,),
    ).fetchall()
    assert len(rows) == 1
    assert rows[0]["recipient_agent_id"] is None
    assert rows[0]["recipient_issue_id"] == issue_id


def test_create_note_deliveries_dedup(temp_db):
    """INSERT OR IGNORE on duplicate rows does not raise and returns 0 for ignored rows."""
    note_id = temp_db.add_note(content="dedup test")
    count1 = temp_db.create_note_deliveries(note_id, to_agents=["agent-x"])
    assert count1 == 1

    # Second call with same agent should be ignored
    count2 = temp_db.create_note_deliveries(note_id, to_agents=["agent-x"])
    assert count2 == 0

    # Only one row should exist
    rows = temp_db.conn.execute("SELECT COUNT(*) FROM note_deliveries WHERE note_id = ?", (note_id,)).fetchone()
    assert rows[0] == 1


def test_materialize_issue_deliveries(temp_db):
    """Issue-following target rows become concrete agent-scoped deliveries."""
    issue_id = temp_db.create_issue("Work item")
    note_id = temp_db.add_note(content="issue note")
    # Create issue-following target
    temp_db.create_note_deliveries(note_id, to_issues=[issue_id])

    materialized = temp_db.materialize_issue_deliveries(issue_id, "agent-abc", "proj")
    assert materialized == 1

    # Should have: original issue-target row + 1 materialized agent+issue row
    rows = temp_db.conn.execute(
        "SELECT recipient_agent_id, recipient_issue_id FROM note_deliveries WHERE note_id = ? AND recipient_agent_id IS NOT NULL",
        (note_id,),
    ).fetchall()
    assert len(rows) == 1
    assert rows[0]["recipient_agent_id"] == "agent-abc"
    assert rows[0]["recipient_issue_id"] == issue_id


def test_materialize_issue_deliveries_dedup(temp_db):
    """Calling materialize_issue_deliveries twice does not create duplicate rows."""
    issue_id = temp_db.create_issue("Work item")
    note_id = temp_db.add_note(content="issue note")
    temp_db.create_note_deliveries(note_id, to_issues=[issue_id])

    count1 = temp_db.materialize_issue_deliveries(issue_id, "agent-abc", "proj")
    count2 = temp_db.materialize_issue_deliveries(issue_id, "agent-abc", "proj")

    assert count1 == 1
    assert count2 == 0  # Already materialized

    # Still only one concrete row
    rows = temp_db.conn.execute(
        "SELECT COUNT(*) FROM note_deliveries WHERE note_id = ? AND recipient_agent_id = ? AND recipient_issue_id = ?",
        (note_id, "agent-abc", issue_id),
    ).fetchone()
    assert rows[0] == 1


def test_get_injectable_deliveries_agent_global(temp_db):
    """Returns agent-global deliveries (recipient_issue_id IS NULL)."""
    note_id = temp_db.add_note(content="global note")
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])

    deliveries, has_more = temp_db.get_injectable_deliveries("agent-1", "some-issue", "proj")
    assert len(deliveries) == 1
    assert deliveries[0]["delivery_id"] is not None
    assert deliveries[0]["note_id"] == note_id
    assert deliveries[0]["content"] == "global note"
    assert deliveries[0]["scope"] == "agent"
    assert not has_more


def test_get_injectable_deliveries_issue_scoped(temp_db):
    """Returns issue-scoped deliveries when issue_id matches."""
    issue_id = temp_db.create_issue("Test")
    note_id = temp_db.add_note(content="issue note")
    # Materialize a delivery for agent+issue
    temp_db.create_note_deliveries(note_id, to_issues=[issue_id])
    temp_db.materialize_issue_deliveries(issue_id, "agent-1", "proj")

    deliveries, _ = temp_db.get_injectable_deliveries("agent-1", issue_id, "proj")
    agent_scoped = [d for d in deliveries if d["scope"] == "issue"]
    assert len(agent_scoped) == 1
    assert agent_scoped[0]["recipient_issue_id"] == issue_id


def test_get_injectable_deliveries_must_read_until_acked(temp_db):
    """must_read deliveries are included until status='acked', excluded after."""
    note_id = temp_db.add_note(content="must read", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])

    # Before acking: should be in results
    deliveries, _ = temp_db.get_injectable_deliveries("agent-1", "issue-x", "proj")
    assert len(deliveries) == 1

    # Ack it
    delivery_id = deliveries[0]["delivery_id"]
    acked = temp_db.mark_delivery_acked(delivery_id, "agent-1")
    assert acked

    # After acking: should NOT be in results
    deliveries, _ = temp_db.get_injectable_deliveries("agent-1", "issue-x", "proj")
    assert len(deliveries) == 0


def test_get_injectable_deliveries_normal_until_read(temp_db):
    """Normal (non-must_read) deliveries excluded once status='read'."""
    note_id = temp_db.add_note(content="normal note", must_read=False)
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])
    ...
    # Mark read
    read = temp_db.mark_delivery_read(delivery_id, "agent-1")
    assert read

    # Normal note after read should be excluded
    deliveries, _ = temp_db.get_injectable_deliveries("agent-1", "issue-x", "proj")
    assert len(deliveries) == 0


def test_get_injectable_deliveries_ordering(temp_db):
    """must_read notes come first; within same must_read, queued before delivered."""
    note_normal = temp_db.add_note(content="normal queued")
    note_must_read = temp_db.add_note(content="must read queued", must_read=True)

    temp_db.create_note_deliveries(note_normal, to_agents=["agent-1"])
    temp_db.create_note_deliveries(note_must_read, to_agents=["agent-1"])

    deliveries, _ = temp_db.get_injectable_deliveries("agent-1", "issue-x", "proj")
    assert len(deliveries) == 2
    # must_read first
    assert deliveries[0]["must_read"] == 1
    assert deliveries[1]["must_read"] == 0


def test_get_injectable_deliveries_cap(temp_db):
    """Caps normal deliveries at max_normal; has_more=True when more exist."""
    note_ids = []
    for i in range(7):
        nid = temp_db.add_note(content=f"normal note {i}")
        temp_db.create_note_deliveries(nid, to_agents=["agent-1"])
        note_ids.append(nid)

    deliveries, has_more = temp_db.get_injectable_deliveries("agent-1", "issue-x", "proj", max_normal=5)
    assert len(deliveries) == 5
    assert has_more is True


def test_mark_delivery_delivered(temp_db):
    """queued -> delivered transition works; idempotent on non-queued."""
    note_id = temp_db.add_note(content="note")
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])

    row = temp_db.conn.execute("SELECT id FROM note_deliveries WHERE note_id = ?", (note_id,)).fetchone()
    delivery_id = row["id"]

    result = temp_db.mark_delivery_delivered(delivery_id)
    assert result is True

    status = temp_db.conn.execute("SELECT status FROM note_deliveries WHERE id = ?", (delivery_id,)).fetchone()["status"]
    assert status == "delivered"

    # Calling again does nothing (already delivered)
    result2 = temp_db.mark_delivery_delivered(delivery_id)
    assert result2 is False


def test_mark_delivery_read(temp_db):
    """delivered -> read, checks agent_id ownership."""
    note_id = temp_db.add_note(content="note")
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])

    row = temp_db.conn.execute("SELECT id FROM note_deliveries WHERE note_id = ?", (note_id,)).fetchone()
    delivery_id = row["id"]
    temp_db.mark_delivery_delivered(delivery_id)

    # Wrong agent cannot mark as read
    result = temp_db.mark_delivery_read(delivery_id, "agent-2")
    assert result is False

    # Correct agent can mark as read
    result = temp_db.mark_delivery_read(delivery_id, "agent-1")
    assert result is True
...
def test_mark_delivery_acked(temp_db):
    """mark_delivery_acked transitions must_read delivery to acked."""
    note_id = temp_db.add_note(content="must read", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])
    ...
    result = temp_db.mark_delivery_acked(delivery_id, "agent-1")
    assert result is True
...
def test_mark_delivery_acked_rejects_non_must_read(temp_db):
    """mark_delivery_acked returns False for normal (non-must_read) notes."""
    note_id = temp_db.add_note(content="normal note", must_read=False)
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])
...
def test_get_required_unacked_deliveries(temp_db):
    """Returns only must_read deliveries that have not been acked."""
    # must_read note for agent global scope
    note_mr = temp_db.add_note(content="must read", must_read=True)
    temp_db.create_note_deliveries(note_mr, to_agents=["agent-1"])

    # Normal note should NOT appear
    note_normal = temp_db.add_note(content="normal", must_read=False)
    temp_db.create_note_deliveries(note_normal, to_agents=["agent-1"])

    results = temp_db.get_required_unacked_deliveries("agent-1", "issue-x")
    assert len(results) == 1
    assert results[0]["note_id"] == note_mr
    assert results[0]["content"] == "must read"


def test_get_required_unacked_deliveries_empty_when_acked(temp_db):
    """Returns empty list after the must_read delivery is acked."""
    note_id = temp_db.add_note(content="must read", must_read=True)
    temp_db.create_note_deliveries(note_id, to_agents=["agent-1"])

    row = temp_db.conn.execute("SELECT id FROM note_deliveries WHERE note_id = ?", (note_id,)).fetchone()
    delivery_id = row["id"]
    temp_db.mark_delivery_acked(delivery_id, "agent-1")

    results = temp_db.get_required_unacked_deliveries("agent-1", "issue-x")
    assert results == []


def test_get_inbox_deliveries_basic(temp_db):
    """Returns all deliveries for agent regardless of status."""
    note1 = temp_db.add_note(content="note 1")
    note2 = temp_db.add_note(content="note 2")
    temp_db.create_note_deliveries(note1, to_agents=["agent-1"])
    temp_db.create_note_deliveries(note2, to_agents=["agent-1"])

    # Also create delivery for a different agent — should not appear
    note3 = temp_db.add_note(content="note 3")
    temp_db.create_note_deliveries(note3, to_agents=["agent-2"])

    results = temp_db.get_inbox_deliveries("agent-1")
...
def test_get_inbox_deliveries_unread_only(temp_db):
    """get_inbox_deliveries with unread_only=True excludes read/acked."""
    note1 = temp_db.add_note(content="queued")
    note2 = temp_db.add_note(content="will be read")
    temp_db.create_note_deliveries(note1, to_agents=["agent-1"])
    temp_db.create_note_deliveries(note2, to_agents=["agent-1"])

    # Mark note2's delivery as read
    row = temp_db.conn.execute("SELECT id FROM note_deliveries WHERE note_id = ?", (note2,)).fetchone()
    temp_db.mark_delivery_read(row["id"], "agent-1")

    results_all = temp_db.get_inbox_deliveries("agent-1", unread_only=False)
