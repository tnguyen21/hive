...
class Orchestrator:
    ...
    def __init__(
        ...
        # Track last DB lease renewal to prevent I/O thrashing on every SSE event
        self._session_last_db_renewal: Dict[str, datetime] = {}
    ...
    def _renew_lease_for_session(self, session_id: str):
        ...
        # Debounce DB writes: only update if enough time has passed since last update
        # Using 10% of lease extension as the debounce interval (default 60s)
        last_db_renewal = self._session_last_db_renewal.get(session_id)
        if last_db_renewal:
            debounce_interval = Config.LEASE_EXTENSION / 10
            if (now - last_db_renewal).total_seconds() < debounce_interval:
                return
        ...
        if agent_id and agent_id in self.active_agents:
            ...
            try:
                ...
                self.db.conn.commit()
                self._session_last_db_renewal[session_id] = now
            except Exception:
    ...
    async def monitor_agent(self, agent: AgentIdentity):
        ...
        try:
            ...
            self._session_last_db_renewal.pop(my_session_id, None)
